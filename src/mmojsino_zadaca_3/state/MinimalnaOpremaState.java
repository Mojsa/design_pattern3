...
public class MinimalnaOpremaState implements State {
        ...
        for (String spec : r.getSpecijalnost()) {
            if (spec.contains("Suho odijelo")) {
                Oprema op = oprema.stream().filter(x -> x.getNazivOpreme().matches(".*(neopren|trilaminat).*") && x.getTemp() <= up.getTemp()
                        && x.getBrojKomada() > 0).max((p1, p2) -> Integer.compare(p1.getTemp(), p2.getTemp())).orElse(null);
                ...
                Oprema racunalo = oprema.stream().filter(x -> x.getNazivOpreme().contains("računalo")
                        && x.getBrojKomada() > 0).max((p1, p2) -> Integer.compare(p1.getBrojKomada(), p2.getBrojKomada())).orElse(null);
                if (racunalo != null) {
    ...
    public String toString() {
        return "Minimalna oprema."; //To change body of generated methods, choose Tools | Templates.
    }
